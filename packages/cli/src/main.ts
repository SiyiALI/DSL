import type { Game } from 'board-race-language';
import { createBoardRaceServices, BoardRaceLanguageMetaData } from 'board-race-language';
...
export const generateAction = async (source: string, destination: string): Promise<void> => {
    const services = createBoardRaceServices(NodeFileSystem).BoardRace;
...
export default function(): void {
    ...
    const fileExtensions = BoardRaceLanguageMetaData.fileExtensions.join(', ');
    program
        .command('generate')
